...
def read_ws(ws,client):
    ...
    try:
        while True:
            # forever listening for messages
            msg = ws.receive()  # receive from web socket
            print("WS RECV: %s" % msg)
            if (msg is not None):
                dictionaryData = json.loads(msg) # parse it into json
                # update the world
                for key in dictionaryData:
                    myWorld.set(key, dictionaryData[key])
            else:
                break
    except:
        '''Done'''
...
def subscribe_socket(ws):
    ...
    # XXX: TODO IMPLEMENT ME
    client = Client() # add new client to client list
    clients.append(client)
    g = gevent.spawn( read_ws, ws, client )    
    try:
        while True:
            # block here
            msg = client.get() # gevent queue   this would be promise.then() call back function in js
            ws.send(msg)  # writes to the web socket
